...
{if(magnetOn != on)		{
	...
	private void ResetPooledObjects(bool destroy = false)
	{
		...
		{
			if ((destroy || !(SprayPaintItem.sprayPaintDecals[num] != null) || !SprayPaintItem.sprayPaintDecals[num].transform.IsChildOf(elevatorTransform)) && !(SprayPaintItem.sprayPaintDecals[num] == null))
			{
		...
		try
		{
			...
			if (ES3.KeyExists("UnlockedShipObjects", GameNetworkManager.Instance.currentSaveFileName))
			{
				...
				for (int i = 0; i < array2.Length; i++)
				{
					...
					{
						if (!unlockablesList.unlockables[array2[i]].alreadyUnlocked)
						{
							unlockablesList.unlockables[array2[i]].hasBeenUnlockedByPlayer = true;
						}
						if (ES3.KeyExists("ShipUnlockStored_" + unlockablesList.unlockables[array2[i]].unlockableName, GameNetworkManager.Instance.currentSaveFileName) && ES3.Load("ShipUnlockStored_" + unlockablesList.unlockables[array2[i]].unlockableName, GameNetworkManager.Instance.currentSaveFileName, defaultValue: false))
						{
							unlockablesList.unlockables[array2[i]].inStorage = true;
						}
						else
						{
				...
				{
					if (!unlockablesList.unlockables[array3[j].unlockableID].spawnPrefab && unlockablesList.unlockables[array3[j].unlockableID].inStorage)
					{
						array3[j].parentObject.disableObject = true;
...
{		try
		{
			...
			SyncShipUnlockablesClientRpc(array, shipRoomLights.areLightsOn, list2.ToArray(), list3.ToArray(), list.ToArray(), list4.ToArray(), list5.ToArray(), list6.ToArray(), vehicleID);
		}
		catch (Exception arg)
		{
	...
	[ClientRpc]
	public void SyncShipUnlockablesClientRpc(int[] playerSuitIDs, bool shipLightsOn, Vector3[] placeableObjectPositions, Vector3[] placeableObjectRotations, int[] placeableObjects, int[] storedItems, int[] scrapValues, int[] itemSaveData, int vehicleID = -1)
{		if (!base.IsServer)
		{
...
{		{
	...
	{
		if (!base.IsServer || base.IsServer)
		{
			return;
		}
		...
		for (int i = 0; i < levels.Length; i++)
		{
			...
			{
				levels[i].moldSpreadIterations++;
				Debug.Log($"Increasing level #{i} {levels[i].PlanetName} mold iterations by 1; risen to {levels[i].moldSpreadIterations}");
				continue;
			}
			float num = ((levels[i] == currentLevel) ? 0.085f : ((terminal.groupCredits < 200 && levels[i].levelID == 12) ? 0.05f : ((!((float)terminal.groupCredits < 500f) || (levels[i].levelID != 7 && levels[i].levelID != 6 && levels[i].levelID < 10) || (currentLevel.levelID != 5 && currentLevel.levelID != 8 && currentLevel.levelID != 4 && currentLevel.levelID > 2)) ? 0.04f : 0.02f)));
